...
## 🛠️ ローカル開発環境の最適化 (Gemini CLI Optimization)

プロジェクトでは `Gemini CLI` の設定を最適化し、エージェント型開発環境（Agentic Workspace）としての機能を強化しています。以下の設定が自動的に適用されます。

### 主な最適化内容

- **コンテキストの統合**: `AGENTS.md` が主要なコンテキストとして自動的に読み込まれ、プロジェクトのルールやアーキテクチャをエージェントが常に把握します。
- **UI/UXの向上**:
  - **Alternate Buffer**: エージェントの出力でターミナル履歴が汚れるのを防ぎます。
  - **ステータス表示**: 思考中や実行中のステータスがタイトルバーに表示されます。
- **安全性とセキュリティ**:
  - **YOLOモード無効化**: 重要なコマンド実行時には必ず承認を求めます。
  - **安全なツールの許可**: `ls`, `grep` などの参照系コマンドは承認なしで実行可能です。

### 設定ファイル

プロジェクト固有の設定は `.gemini/settings.json` に保存されています。チーム全体で統一された開発体験を提供するため、このファイルはGitで管理されています。
