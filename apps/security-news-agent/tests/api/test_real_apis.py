...
class TestRealAPIIntegration:
    ...
    def test_tavily_client_real_search(self, api_config):
        ...
        try:
            ...
            print(f"✅ Tavily API test successful. Found {len(result['results'])} results.")
    ...
    def test_tavily_client_collect_context_limited(self, api_config):
        ...
        try:
            ...
            print(f"✅ Context collection test successful. Found {len(results)} unique results.")
    ...
    def test_minimal_workflow_execution(self, limited_config, tmp_path):
        ...
        try:
            ...
            if "sources" in result and result["sources"]:
                print(f"✅ News collection successful. Found sources: {list(result['sources'].keys())}")
                
                # If we have sources, check other steps
                if "outline" in result and result["outline"]:
                    print(f"✅ Outline generation successful. Items: {len(result['outline'])}")
                
                if "slide_md" in result and result["slide_md"]:
                    print(f"✅ Slide generation successful. Length: {len(result['slide_md'])} chars")
                    ...
                    if render_result["success"]:
                        print(f"✅ Report rendering successful: {render_result['markdown_path']}")
                        ...
                        print(f"⚠️ Report rendering failed: {render_result.get('error', 'Unknown error')}")
                else:
                    print("⚠️ Slide generation did not complete")
            else:
                print("⚠️ No news sources found - this may be normal if no recent news matches the query")
            ...
            if result.get("error"):
                ...
                if "api" in result["error"].lower() or "rate" in result["error"].lower():
                    pytest.skip(f"API-related error (expected): {result['error']}")
            ...
            if any(term in error_msg for term in ["rate limit", "quota", "timeout", "network"]):
...
class TestAPIConfiguration:
    ...
    def test_search_queries_configuration(self):
        ...
        try:
            ...
            print(f"✅ Search queries configuration valid. Found {len(queries)} queries.")
...
def run_api_tests():
    ...
    print("Note: These tests require valid API keys and may consume API quotas.")
