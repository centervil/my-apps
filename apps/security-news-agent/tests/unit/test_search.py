...
from tests.fixtures.mock_data import MOCK_SEARCH_QUERIES, MOCK_TAVILY_RESPONSE
...
class TestTavilyClient:
    ...
    def test_collect_context_dict_queries(self, mock_search):
        ...
        assert calls[0][1]["include_domains"] == [
            "thehackernews.com",
            "bleepingcomputer.com",
        ]
        assert calls[1][1]["include_domains"] == [
            "krebsonsecurity.com",
            "darkreading.com",
        ]
    ...
    def test_collect_context_search_failure(self, mock_search):
        ...
        # Second query failed but didn't crash
        assert len(result["query2"]) == 0
    ...
    def test_filter_results_by_keywords(self):
        ...
        client = TavilyClient("test-api-key")
        filtered = client.filter_results_by_keywords(
    ...
    def test_filter_results_case_sensitive(self):
        ...
        filtered_insensitive = client.filter_results_by_keywords(
            context, ["security"]
        )
        ...
        # "security" != "SECURITY"
        assert len(filtered_sensitive["query1"]) == 0
