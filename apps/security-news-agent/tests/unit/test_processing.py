...
class TestWorkflowNodes:
    ...
    def test_collect_info_success(self, mock_initial_state, mock_config):
        ...
        mock_tavily.format_context_as_markdown.return_value = (
            "### Query: test\n- Article 1"
        )
        mock_tavily.get_total_results_count.return_value = 3

        result = WorkflowNodes.collect_info(
            mock_initial_state, mock_tavily, mock_config
        )
    ...
    def test_make_toc_json_parse_error(self, mock_initial_state):
        ...
        with patch(
            "security_news_agent.processing.nodes.strip_bullets"
        ) as mock_strip:
    ...
    def test_make_toc_default_fallback(self, mock_initial_state):
        ...
        with patch(
            "security_news_agent.utils.helpers.strip_bullets"
        ) as mock_strip:
    ...
    def test_write_slides_success(self, mock_initial_state):
        ...
        with patch(
            "security_news_agent.processing.nodes.today_iso"
        ) as mock_today:
...
class TestSecurityNewsWorkflow:
    ...
    def test_run_success(self, mock_state_graph, mock_config):
        ...
        mock_state_graph.return_value.compile.return_value = (
            mock_graph_instance
        )
