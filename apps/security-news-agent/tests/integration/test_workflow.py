...
class TestEndToEndWorkflow:
    ...
    def mock_tavily_client(self):
        ...
        client.format_context_as_markdown.return_value = self._create_mock_context_markdown()
...
"""
    ...
    def test_complete_workflow_success(self, integration_config, mock_tavily_client, temp_output_dir):
...
"""),
            # TOC response
            Mock(content='{"toc": ["Executive Summary", "Critical Vulnerabilities", "Data Breaches", "Malware Threats", "Recommendations"]}'),
            # Slides response
        ...
        with patch('security_news_agent.processing.workflow.ChatGoogleGenerativeAI') as mock_llm_class:
            ...
            # Create workflow
            workflow = SecurityNewsWorkflow(integration_config, mock_tavily_client)
            ...
            assert mock_llm.invoke.call_count >= 3  # At least outline, TOC, slides
    
    def test_workflow_with_tavily_error(self, integration_config, temp_output_dir):
        ...
        mock_tavily_client.collect_context.side_effect = Exception("Tavily API Error")
    ...
    def test_workflow_with_llm_error(self, integration_config, mock_tavily_client, temp_output_dir):
    ...
    def test_workflow_retry_logic(self, integration_config, mock_tavily_client, temp_output_dir):
        ...
        mock_llm_responses = [
            ...
            Mock(content='{"score": 6.0, "pass": false, "feedback": "Needs improvement"}'),
            # Second attempt - TOC
    ...
    def test_workflow_max_attempts_reached(self, integration_config, mock_tavily_client, temp_output_dir):
        ...
        failing_responses = [
            ...
            Mock(content='{"score": 5.0, "pass": false, "feedback": "Poor quality"}')  # evaluation
        ]
    ...
    def test_complete_workflow_with_output_rendering(self, integration_config, mock_tavily_client, temp_output_dir):
        ...
        with patch('security_news_agent.processing.workflow.ChatGoogleGenerativeAI') as mock_llm_class:
            ...
            render_result = renderer.save_and_render(result["slide_md"], result.get("title", "Test Report"))
    ...
    def test_workflow_prerequisites_validation(self, integration_config, mock_tavily_client):
    ...
    def test_workflow_state_management(self, integration_config, mock_tavily_client):
        ...
        with patch('security_news_agent.processing.workflow.ChatGoogleGenerativeAI'):
            ...
            assert initial_state["topic"] == "Daily Cybersecurity Threat Briefing"
    ...
    def test_error_propagation_through_workflow(self, integration_config, temp_output_dir):
        ...
        mock_tavily_client.format_context_as_markdown.return_value = "### Query: test\n- Article 1"
        ...
        with patch('security_news_agent.processing.workflow.ChatGoogleGenerativeAI') as mock_llm_class:
            mock_llm = Mock()
            mock_llm.invoke.side_effect = mock_llm_responses + [Exception("Slides generation failed")]
            mock_llm_class.return_value = mock_llm
