...
from ..utils.error_handling import APIError, RateLimitError, log_api_call
...
class TavilyClient:
    ...
    def search(
        ...
        try:
            ...
            logger.info(f"Found {len(data.get('results', []))} results for query: '{query}'")
            return data
    ...
    def collect_context(
        ...
        for query_config in queries:
            ...
            try:
                ...
                logger.info(f"Collected {len(query_results)} unique results for: '{query_text}'")
