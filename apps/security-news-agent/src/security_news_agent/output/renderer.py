...
from ..utils.helpers import today_iso, slugify_en
...
class ReportRenderer:
    ...
    def __init__(self, config: AgentConfig, output_dir: str = "slides"):
        ...
        logger.info(f"Initialized ReportRenderer with output_dir: {self.output_dir}")
        if self.marp_cli:
            logger.info(f"Marp CLI found at: {self.marp_cli}")
        else:
            logger.warning("Marp CLI not found - rendering to non-markdown formats will be unavailable")
    ...
    def ensure_output_directory(self) -> Path:
        ...
        try:
            ...
            raise FileOperationError(f"Failed to create output directory {self.output_dir}: {e}")
    ...
    def save_markdown(self, content: str, title: str, filename: str = None) -> Path:
    ...
    def render_with_marp(self, md_path: Path, output_format: str) -> Optional[Path]:
    ...
    def save_and_render(self, content: str, title: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.config.slide_format and self.config.slide_format in {"pdf", "png", "html"}:
                try:
                    rendered_path = self.render_with_marp(md_path, self.config.slide_format)
                    if rendered_path:
                        result["rendered_path"] = str(rendered_path)
                        logger.info(f"Report saved and rendered successfully")
                    else:
                        logger.warning("Markdown saved but rendering failed")
                        result["error"] = "Rendering failed but markdown was saved"
    ...
    def validate_markdown_content(self, content: str) -> Dict[str, Any]:
        ...
        if len(content) < 100:
            ...
            validation["warnings"].append("Content is very long, may cause rendering issues")
