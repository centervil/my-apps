        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                logger.log(
                    ...
                    extra={"error_details": e.details, "function": func.__name__}
                )
                if reraise:
                    raise
...
class ErrorCollector:
    ...
    def add_warning(self, message: str, context: str = None):
        ...
        logger.warning(f"Warning collected: {message}", extra={"context": context})
    ...
    def raise_if_errors(self, message: str = None):
        ...
        """
        if self.has_errors():
            error_message = message or f"Processing failed with {len(self.errors)} errors"
            raise ProcessingError(error_message, "error_collection", summary=self.get_summary())
