...
class WorkflowNodes:
    ...
    def collect_info(
        ...
        try:
            ...
            sources = tavily_client.collect_context(
                queries, max_per_query=5, default_time_range="day"
            )
            ...
            logger.info(
                f"Collected {total_results} news articles from {len(sources)} queries"
            )
            ...
            return {
                ...
                "log": log_message(
                    state,
                    f"[collect_info] Found news from {len(sources)} queries, {total_results} total results.",
                ),
            }
            ...
            return {
                "error": error_msg,
                "log": log_message(
                    state, f"[collect_info] UNEXPECTED EXCEPTION {e}"
                ),
            }
    ...
    def make_outline(state: State, llm: ChatGoogleGenerativeAI) -> Dict:
        ...
        prompt = f"""
System: You are a senior cybersecurity analyst. Your task is to create a \
presentation outline based on the latest security news.

User: Based on the following "Latest News Summary (with sources)", identify \
the 5 most critical security topics for a daily briefing. Present them as a \
concise bulleted list. Include URLs in your points.
...
"""
        ...
        try:
            ...
            return {
                "outline": bullets,
                "log": log_message(
                    state, f"[outline] Generated {len(bullets)} outline items"
                ),
            }
...
"""
        ...
        try:
            ...
            return {
                ...
                "error": "",
                "log": log_message(
                    state, f"[toc] Generated {len(toc)} chapters"
                ),
            }
    ...
    def write_slides(state: State, llm: ChatGoogleGenerativeAI) -> Dict:
        ...
        prompt = f"""
System: You are a senior cybersecurity analyst creating a presentation in \
Marp Markdown format.
Do not wrap the output in a code block. Do not include slide separators (---).
Each slide must start with an H2 header (##). Do not write the presenter's \
name on the title slide.
IMPORTANT: Base your writing ONLY on the facts provided in the "Latest News \
Summary" below. Do not include information not present in the summary.
...
"""
        ...
        try:
            ...
            logger.info(
                f"Generated slide content ({len(slide_md)} characters)"
            )
            return {
                "slide_md": slide_md,
                "title": title,
                "error": "",
                "log": log_message(
                    state, f"[slides] generated ({len(slide_md)} chars)"
                ),
            }
    ...
    def evaluate_slides(
        ...
        if not slide_md.strip():
            ...
            return {
                ...
                "log": log_message(
                    state, "[evaluate] No slide content available"
                ),
            }
...
"""
        ...
        try:
            ...
            logger.info(
                f"Evaluation complete: score={score:.2f}, passed={passed}, attempts={attempts}"
            )
            ...
            return {
                ...
                "log": log_message(
                    state,
                    f"[evaluate] score={score:.2f} pass={passed} attempts={attempts}",
                ),
            }
            ...
            return {
                ...
                "attempts": attempts,
                "log": log_message(
                    state, f"[evaluate] parsing failed, attempts={attempts}"
                ),
            }
        except Exception as e:
    ...
    def route_after_eval(state: State, max_attempts: int = 3) -> str:
        ...
        if attempts >= max_attempts:
            logger.info(
                f"Max attempts ({max_attempts}) reached, proceeding to save"
            )
            return "ok"
        ...
        if passed:
            ...
            logger.info(
                f"Evaluation failed (attempt {attempts}/{max_attempts}), retrying"
            )
            return "retry"
