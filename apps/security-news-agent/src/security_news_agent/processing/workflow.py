...
class SecurityNewsWorkflow:
    ...
    def __init__(self, config: AgentConfig, tavily_client: TavilyClient, llm_client: Any = None):
        ...
        logger.info(f"Initialized SecurityNewsWorkflow with model: {config.gemini_model_name}")
    ...
    def _build_graph(self) -> StateGraph:
        ...
        graph_builder.add_node("evaluate_slides", self._evaluate_slides_wrapper)
    ...
    def _collect_info_wrapper(self, state: State) -> Dict:
        ...
        return WorkflowNodes.collect_info(state, self.tavily_client, self.config)
    ...
    def _evaluate_slides_wrapper(self, state: State) -> Dict:
        ...
        return WorkflowNodes.evaluate_slides(state, self.llm, self.max_attempts)
    ...
    def run(self, initial_state: State = None, config: RunnableConfig = None) -> Dict:
        ...
        try:
            ...
            if result.get("error"):
                ...
                logger.info(f"Workflow completed successfully. Score: {result.get('score', 'N/A')}")
