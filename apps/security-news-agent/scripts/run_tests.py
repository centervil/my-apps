...
def run_command(cmd, description):
    ...
    try:
        ...
        print("ERROR: Command not found. Make sure poetry is installed.")
        return False


def main():
    parser = argparse.ArgumentParser(
        description="Run tests for security news agent"
    )
    parser.add_argument(
    ...
    if args.all or args.lint:
        # Run linting
        if not run_command(
            ["poetry", "run", "flake8", "src/", "tests/"], "Linting (flake8)"
        ):
            success = False
        if not run_command(
    ...
    if args.unit:
        ...
        [args.unit, args.integration, args.coverage, args.lint, args.format]
    ):
        # Run all tests
