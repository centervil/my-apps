...
`);
  ...
  try {
    if (fs.existsSync(packageJsonPath)) {
      console.log(`Running: pnpm --filter ${projectName} test\n`);
      const testResult = await $`pnpm --filter ${projectName} test`.nothrow();
      console.log(testResult.stdout);
      if (testResult.exitCode !== 0) {
        console.log(`\n**Test Failed** (Exit Code: ${testResult.exitCode})\n`);
        console.log(testResult.stderr);
      } else {
        console.log(`\n**Test Passed**\n`);
      }
    } else if (fs.existsSync(pyprojectTomlPath)) {
      console.log(`Running: poetry run pytest (in ${projectPath})\n`);
      const testResult = await $`cd ${projectPath} && poetry run pytest`.nothrow();
      console.log(testResult.stdout);
      if (testResult.exitCode !== 0) {
        console.log(`\n**Test Failed** (Exit Code: ${testResult.exitCode})\n`);
        console.log(testResult.stderr);
      ...
      console.log(
        'No test configuration found (package.json or pyproject.toml missing). Skipping tests.\n',
      );
    }
  } catch (error) {
